...
const positiveAmountArb = fc.float({ min: Math.fround(0.01), max: Math.fround(9999), noNaN: true });
...
describe('UI form behavior (Properties 1–4)', () => {
  ...
  describe('Property 3: Invalid amount is rejected', () => {
    ...
    it('submitting with a non-numeric amount string leaves the list unchanged', () => {
      // Feature: expense-budget-visualizer, Property 3: Invalid amount is rejected
      const nonNumericArb = fc.string({ minLength: 1, maxLength: 20 })
        .filter(s => {
...
const transactionArb2 = fc.record({
  ...
  amount:   fc.float({ min: Math.fround(0.01), max: Math.fround(9999), noNaN: true }),
      ...
      fc.property(
        ...
        (transactions) => {
          ...
          transactions.forEach((t, i) => {
            ...
            expect(item.querySelector('.txn-name').textContent).toContain(t.name);
            expect(item.querySelector('.txn-amount').textContent).toContain(t.amount.toFixed(2));
            expect(item.querySelector('.txn-category').textContent).toContain(t.category);
          });
        }
      ),
      { numRuns: 100 }
      ...
      fc.property(
        ...
        (transactions) => {
          ...
          for (let i = 1; i < renderedAmounts.length; i++) {
            expect(renderedAmounts[i - 1]).toBeLessThanOrEqual(renderedAmounts[i]);
          }
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (transactions) => {
          ...
          for (let i = 1; i < renderedAmounts.length; i++) {
            expect(renderedAmounts[i - 1]).toBeGreaterThanOrEqual(renderedAmounts[i]);
          }
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (transactions) => {
          ...
          const list = document.getElementById('transaction-list');
          const items = [...list.querySelectorAll('li[data-id]')];
          const renderedCategories = items.map(li =>
            li.querySelector('.txn-category').textContent
          );

          for (let i = 1; i < renderedCategories.length; i++) {
            expect(renderedCategories[i - 1].localeCompare(renderedCategories[i])).toBeLessThanOrEqual(0);
          }
        }
      ),
      { numRuns: 100 }
    ...
    fc.assert(
      fc.property(
        ...
        (transactions, { field, dir }) => {
          ...
          expect(stored.map(t => t.id)).toEqual(transactions.map(t => t.id));
...
describe('Custom category (Task 9.2)', () => {
  ...
  it('adds a new category to state and the select element', () => {
    ...
    const options = [...document.getElementById('category').options].map(o => o.value);
  ...
  it('property: any unique non-empty category string is added exactly once', () => {
    ...
    const uniqueCatArb = fc.stringOf(
...
describe('renderMonthlySummary (Task 10.1)', () => {
  ...
  it('renders month label for each distinct month', () => {
    const txns = [
      ...
      { id: '2', name: 'Coffee', amount: 5,  category: 'Food',      date: '2024-06-01' },
    ];
    renderMonthlySummary(txns);
    const content = document.getElementById('monthly-summary-content');
  ...
  it('escapes HTML in category names', () => {
    const txns = [
      { id: '1', name: 'Test', amount: 5, category: '<script>alert(1)</script>', date: '2024-06-01' },
    ];
    renderMonthlySummary(txns);
    const content = document.getElementById('monthly-summary-content');
...
describe('Toggle Monthly Summary (Task 10.2)', () => {
  ...
  it('section is hidden by default', () => {
    expect(document.getElementById('monthly-summary').hasAttribute('hidden')).toBe(true);
  });

  it('clicking the button shows the section', () => {
    document.getElementById('toggle-monthly-summary').click();
    expect(document.getElementById('monthly-summary').hasAttribute('hidden')).toBe(false);
  });
...
describe('Task 13.3: Integration – full add-then-delete cycle', () => {
  ...
  it('adding a transaction updates balance and list, then deleting restores empty state', () => {
    ...
    expect(balanceEl.textContent).toBe('4.50');
