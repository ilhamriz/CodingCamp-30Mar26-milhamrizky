            ...
            for (const entry of result) {
              for (const [cat, total] of Object.entries(entry.totals)) {
                const expected = transactions
                  .filter(
                    (t) =>
                      t.date.slice(0, 7) === entry.month && t.category === cat,
                  )
        ...
        fc.property(
          ...
          (transactions) => {
            ...
            for (let i = 1; i < result.length; i++) {
              expect(
                result[i - 1].month.localeCompare(result[i].month),
    ...
    fc.assert(
      fc.property(
        ...
        (transactions) => {
          ...
          for (let i = 1; i < sorted.length; i++) {
            expect(sorted[i - 1].amount).toBeGreaterThanOrEqual(
              sorted[i].amount,
            );
          }
        },
    ...
    fc.assert(
      fc.property(
        ...
        (transactions) => {
          ...
          for (let i = 1; i < sorted.length; i++) {
            expect(
              sorted[i - 1].category.localeCompare(sorted[i].category),
            ).toBeLessThanOrEqual(0);
          }
        },
      ),
...
// ─── formatCurrency / parseCurrencyAmount (inlined from app.js) ───────────────
...
describe("Property 1: formatCurrency output format", () => {
  it("for any positive integer, formatCurrency output matches /^Rp[\\u00a0\\s][\\d.]+$/", () => {
    // Feature: rupiah-currency-update, Property 1: formatCurrency output format
    fc.assert(
...
describe("Property 2: formatCurrency / parseCurrencyAmount round-trip", () => {
  it("for any positive integer n, parseCurrencyAmount(formatCurrency(n)) === n", () => {
    // Feature: rupiah-currency-update, Property 2: formatCurrency / parseCurrencyAmount round-trip
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 10_000_000 }), (n) => {
        expect(parseCurrencyAmount(formatCurrency(n))).toBe(n);
      }),
      { numRuns: 100 },
    );
  });
